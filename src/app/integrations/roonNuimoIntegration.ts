...
export class RoonNuimoIntegration implements IntegrationInterface {
    ...
    constructor(options: { nuimo: string; zone: string; output: string; broker: Broker }) {
        ...
        this.topicsToSubscribe = [this.operationTopic, this.roonStateTopic, this.roonVolumeTopic];
    ...
    up(): Promise<any> {
        this.resolveRoute();
        return this.broker.subscribe(this.topicsToSubscribe);
    }
    ...
    private resolveRoute() {
        this.broker.on("message", (topic: string, payload: Buffer, _: IPublishPacket) => {
            switch (topic) {
                ...
                case this.roonVolumeTopic:
                    this.nuimoReaction(JSON.stringify({status: "volumeChange", percentage: payload.toString()}));
                    break;
                default:
                    break;
            }
